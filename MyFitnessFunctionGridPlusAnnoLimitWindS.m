function [E_load, E_pv, E_bat, E_grid, d, Costo, andamento_charge, E_wind] = MyFitnessFunctionGridPlusAnnoLimitWindS(x1, x2, x3)
    ...
    for k = 1 : 12
        ...
        for j = 1 : length(P_load(k).month)
            ...
            E_wind(row_accumulate + j,:) = P_wind(row_accumulate + j, :) * x3 * delta_t;
            for i = 1 : 24
                Energy = E_load(row_accumulate + j,i) - E_pv(row_accumulate + j,i) - E_wind(row_accumulate + j, i);
